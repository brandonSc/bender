...
If runtime status shows work in progress, report it accurately.`,
  ...
  private async handleSlackWork(
    ...
    const prompt = [
      ...
      "",
      `## Instructions`,
      `**EXECUTE — do not re-plan.** A plan was already discussed and approved in the conversation above.`,
      `Read the conversation history carefully to understand what was agreed upon, then implement it.`,
      `Do NOT propose a new plan or ask "want me to go ahead?" — the human already said go ahead.`,
      `Make the changes, commit, and push. You have full tool access.`,
      ``,
