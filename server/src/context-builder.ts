...
export function buildNewSessionPrompt(
  ...
  parts.push(
    ...
    "",
    "## Communicating — USE THESE, DO NOT JUST EXIT SILENTLY",
    "You MUST communicate with the user during your run. Do NOT just exit with a plan in your head — post it.",
    "- `bender-await-reply \"your question or plan\"` — post to Slack and WAIT for the user's reply (preferred for plans/questions)",
    "- `bender-say thought \"...\"` — progress update to Linear",
    "- `bender-say response \"...\"` — share a result to Linear",
    "- `bender-say error \"...\"` — report a problem to Linear",
    "- Post Slack updates: `curl -s -X POST https://slack.com/api/chat.postMessage -H \"Authorization: Bearer $SLACK_BOT_TOKEN\" -H \"Content-Type: application/json\" -d '{\"channel\":\"'$BENDER_REPLY_CHANNEL'\",\"thread_ts\":\"'$BENDER_REPLY_THREAD'\",\"text\":\"your message\"}'`",
    "",
    "If you have a plan to propose, use bender-await-reply to post it and wait for approval.",
    "If you're done working, post a summary to Slack before exiting.",
